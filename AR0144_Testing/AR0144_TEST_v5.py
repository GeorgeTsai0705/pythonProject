...
def cubic_fitting(roi_avg):
    global Standard_Wavelength

    width = 3.5
    prominence = 7.0
    height = 800
    peaks, fwhm_values = calculate_fwhm(roi_avg, width, prominence, height)
    ...
    fwhm_table = create_table(result_window)
    ...
    update_table_with_fwhm_nm(fwhm_table, peaks, best_comb, FWHM_convert)

class App:
    def __init__(self, window, window_title):
        self.window = window
        self.window.title(window_title)
        self.ROI = 480  # Initial ROI value
    ...
    def create_widgets(self):
        ...
        self.cubic_fitting_button = ttk.Button(self.buttons_frame, text="Cubic Fitting", command=self.perform_cubic_fitting)
