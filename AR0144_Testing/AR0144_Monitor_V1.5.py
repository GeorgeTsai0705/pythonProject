...
class App:
    ...
    def create_buttons(self):
        ...
        self.snapshot_button = ttk.Button(self.buttons_frame, text="Snapshot", command=self.snapshot_spectrum)
    ...
    def snapshot_spectrum(self):
        # Check if there is current spectrum data available
        if not hasattr(self, 'roi_avg'):
            messagebox.showwarning("Warning", "No spectrum data available to snapshot.")
            return

        # Determine x-axis based on user selection
        ...
        # Get a copy of the current spectrum data
        current_spectrum = self.roi_avg.copy()

        # Subtract the baseline (for example, the average of the first 100 data points)
        baseline = np.average(current_spectrum[:100])
        baseline_corrected_spectrum = current_spectrum - baseline

        # Plot the snapshot on the existing axes
        # Use a dashed line style and a label (timestamp) to distinguish snapshots
        timestamp = datetime.now().strftime('%H:%M:%S')
        self.ax.plot(x_data, baseline_corrected_spectrum, linestyle='--', linewidth=1,
                     label=f"Snapshot {timestamp}")

        # Optionally, update the legend so the snapshot is visible
        self.ax.legend()

        # Redraw the canvas so the snapshot appears immediately
        self.canvas.draw()
